...
parser.add_argument('--knn_embedding_mode', type=str, default='cls',
                    choices=['cls', 'emotion_clause', 'cause_clause'],
                    help='KNN 使用的 embedding 類型: cls=傳統[CLS], emotion_clause=情緒子句聚合, cause_clause=原因子句聚合')
...
if opt.save_path == 'prompt_ECPE_few_shot_ST' and not opt.test_only:
    ...
    opt.save_path = (
        f"prompt_ECPE_few_shot_ST_{timestamp}_{dataset_name}_f{opt.start_fold}-{opt.end_fold}_"
        f"i{opt.training_iter}_lr{lr_str}_bs{opt.batch_size}_"
        f"wd{opt.weight_decay}_"
        f"bert-base-chinese_{threshold_str}{selector_str}"
        f"{nest_multiplier_str}_gamma{opt.gamma}_reg{lambda_str}_st{opt.self_training_rounds}_"
        f"ste{opt.st_training_epochs}_seed{opt.seed}_{retain_pseudo_str}_CE"
    )
...
def generate_experiment_folder_name(opt, bert_path):
    ...
    folder_name = (
        f"UECA-CE_ST_{timestamp}_{dataset_name}_f{opt.start_fold}-{opt.end_fold}_"
        f"i{opt.training_iter}_lr{lr_str}_bs{opt.batch_size}_"
        f"wd{opt.weight_decay}_"
        f"{model_name}_{threshold_str}{selector_str}"
        f"{nest_multiplier_str}_gamma{opt.gamma}_reg{lambda_str}_st{opt.self_training_rounds}_"
        f"ste{opt.st_training_epochs}_seed{opt.seed}_test_model_type_{opt.test_model_type}"
    )
    return folder_name
                            ...
                            for j, pos in enumerate(mask_positions):
                                ...
                                if mask_type == 0:
                                    ...
                                    if cause_predictions.get(count_sentence, False):
                                        ...
                                        case.append(3187)
                                        candidate_logits = logits[i, pos_idx, case]
                                        selected_idx = torch.argmax(candidate_logits).item()
                                        pred_token_id = case[selected_idx]
                                        pseudo_label.append(pred_token_id if isinstance(pred_token_id, int) else pred_token_id.item())
                                    else:
                                        pseudo_label.append(3187)
