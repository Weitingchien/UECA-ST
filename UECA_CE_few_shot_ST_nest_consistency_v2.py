...
def _compute_label_distribution_from_logits(logits, input_ids, mask_index, mask_token_id, yes_token_id, no_token_id, debug=False, doc_id=None, tokenizer=None):
    ...
    # DEBUG: 印出標題
    mask_name = {0: '情緒', 1: '原因', 2: '配對'}.get(mask_index, f'slot{mask_index}')
    if debug:
...
def generate_pseudo_labels_in_batches(model, tokenizer, inputs_array, window_size, device, threshold=None, batch_size=8):
    """分批運算未標註資料，產生對應的偽標籤序列"""
    ...
    with torch.no_grad():
        for start in range(0, len(inputs_array), batch_size):
            ...
            batch_inputs = torch.tensor(inputs_array[start:end], dtype=torch.long, device=device)
            _, logits = model(batch_inputs, labels=None)
            probs = F.softmax(logits, dim=-1).cpu().numpy()
            for doc_probs, doc_input in zip(probs, inputs_array[start:end]):
                pseudo_tokens = generate_pseudo_label_tokens(
                    doc_probs,
                    doc_input,
                    mask_token_id,
                ...
                pseudo_labels.append(pseudo_tokens)
...
def apply_task_consistency_filter(
    ...
    # 並要求結構一致: 非原因句不可有配對；原因句的配對需在合法候選集合
    slot_ensemble_valid = False
    slot_ensemble_reject_reason = None
    slot_ensemble_pseudo = None
    if consistency_rule in ('slot_expert_conservative', 'somc'):
        if not (len(emo_pseudo) == len(cause_pseudo) == len(pair_pseudo)):
            slot_ensemble_reject_reason = 'length_mismatch'
        elif len(emo_pseudo) % 3 != 0:
            slot_ensemble_reject_reason = 'mask_triplet_mismatch'
        else:
            yes_token_id, _ = get_binary_token_ids(tokenizer)
            no_pair_token_id = tokenizer.convert_tokens_to_ids('无')
            pair_label_index = get_label_index()

            merged = np.array(emo_pseudo, dtype=np.int64).copy()
            merged[1::3] = np.array(cause_pseudo, dtype=np.int64)[1::3]
            merged[2::3] = np.array(pair_pseudo, dtype=np.int64)[2::3]

            num_clauses = len(merged) // 3
            for clause_idx in range(1, num_clauses + 1):
                cause_pos = (clause_idx - 1) * 3 + 1
                pair_pos = cause_pos + 1
                cause_token = int(merged[cause_pos])
                pair_token = int(merged[pair_pos])

                if cause_token != yes_token_id:
                    if pair_token != no_pair_token_id:
                        slot_ensemble_reject_reason = f'non_cause_with_pair_clause_{clause_idx}'
                        break
                    merged[pair_pos] = no_pair_token_id
                    continue

                valid_pair_candidates = _build_pair_candidate_ids(
                    clause_idx,
                    window_size,
                    pair_label_index,
                    no_pair_token_id,
                )
                if pair_token not in valid_pair_candidates:
                    slot_ensemble_reject_reason = f'invalid_pair_candidate_clause_{clause_idx}'
                    break

            if slot_ensemble_reject_reason is None:
                slot_ensemble_valid = True
                slot_ensemble_pseudo = merged
    ...
    if consistency_debug_records is not None:
        consistency_debug_records.append({
            ...
            "slot_ensemble_valid": bool(slot_ensemble_valid),
            "slot_ensemble_reject_reason": slot_ensemble_reject_reason,
            ...
            "slot_ensemble_pseudo_ids": slot_ensemble_pseudo.tolist() if slot_ensemble_pseudo is not None else None,
        })

    if consistency_rule in ('slot_expert_conservative', 'somc'):
        if slot_ensemble_valid and slot_ensemble_pseudo is not None:
            consistency_stats["accept"] += 1
            return True, slot_ensemble_pseudo
    else:
        if all_equal:
...
parser.add_argument('--consistency_rule', type=str, default='all_equal',
                    choices=['all_equal', 'somc', 'slot_expert_conservative'],
                    help='一致性規則: all_equal=三模型整串相同才通過; somc(Subtask Optimal Model Collaboration)=三模型分工拼接並做保守衝突檢查')
...
# 向下相容: 舊名稱 slot_expert_conservative 統一映射為 somc
if opt.consistency_rule == 'slot_expert_conservative':
    opt.consistency_rule = 'somc'
...
if opt.save_path == 'prompt_ECPE_few_shot_ST' and not opt.test_only:
    ...
    if opt.consistency_pseudo:
        consistency_str = "consistency" if opt.consistency_rule == 'all_equal' else f"consistency_{opt.consistency_rule}"
    else:
        consistency_str = ""
...
def _run_main(save_path):
    ...
    # 載入模型
        ...
        for self_round in range(opt.self_training_rounds):
            ...
            with torch.no_grad():
                ...
                consistency_enabled = bool(
                    opt.consistency_pseudo and (opt.consistency_rule != 'all_equal' or opt.consistency_require_all_equal)
                )  # 是否啟用一致性過濾
                ...
                for batch_idx, batch in enumerate(combined_loader):
                    ...
                    if pseudo_mask.sum() > 0:
                        ...
                        if opt.nest_loss_mode == 'nest':
                            ...
                            loss_p = loss_p / pseudo_mask.sum() # pseudo_mask.sum(): 批次中偽標籤樣本的數量
                    ...
                    if opt.nest_loss_mode == 'nest':
                        ...
                        total_loss = opt.gamma * loss_l + (1 - opt.gamma) * loss_p
                    else:
        ...
        if os.path.exists(test_checkpoint_path):
            ...
            print(f"找不到測試模型 {test_checkpoint_path}，將使用當前模型狀態進行評估")
            test_model = model
