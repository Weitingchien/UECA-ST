...
class MyDataset(Dataset):
    def __init__(self, input_file, test=False, tokenizer=None):
        ...
        for doc in data:
            ...
            cnt_pair_gt = len(set(tuple(pair) for pair in pairs))
            ...
            if count_len > 512:
                print("Over limit length{} document{}".format(count_len, doc_id))
                cnt_over_limit += 1
            mask_full_document = \
                    ...
                    with torch.autograd.set_detect_anomaly(True):
                        ...
                        if index % 20 == 0:
                            p_emotion, r_emotion, f_emotion, p_cause, r_cause, f_cause, p_pair, r_pair, f_pair = \
                                crf_prompt(logits.cpu(), label.cpu(), x_bert.cpu(), gt_emotion, gt_cause, gt_pair, save_path="evaluation_results.txt")
                            print(
