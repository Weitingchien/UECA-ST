...
def count_ecpe_docs(file_path):
    """Count ECPE-format document blocks by following doc_len headers."""
    import re
    ...
    while i < len(lines):
        ...
        if i + 2 + doc_len > len(lines):
            raise ValueError(f"Truncated document {doc_id} at line {i + 1}")
...
def main():
    print("Run English preprocessing blocks that were commented in original en_dataset_conversion.py")

    # EN-ECPE: generate *_num.txt for Englishnovel subdomains.
    ...
    # EN-ECPE: combine Englishnovel subdomains into enecpe_num.txt.
    ...
    # RECCON: combine DailyDialog train/test/valid into reccon_num.txt.
    ...
    # Count the two files requested by the user.
    targets = [
        ('EN-ECPE', PROJECT_ABSOLUTE_PATH + '/domains/Englishnovel_multiple/enecpe_num.txt', 1226),
        ('RECCON', PROJECT_ABSOLUTE_PATH + '/domains/Englishnovel_multiple/reccon_num.txt', 780),
    ]
    for name, path, expected in targets:
        physical, unique_ids = count_ecpe_docs(path)
        print(f"{name}: physical_doc_blocks={physical}, unique_doc_ids={unique_ids}, paper_expected={expected}, match={physical == expected}")
