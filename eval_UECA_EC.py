...
def save_ground_truth_as_text(gt_file, pred_file, output_file):
    ...
    # Step 2: 處理 gt_file 並依 cause 對應輸出 是 <pos> 是 / 非 无 非
...
def evaluate(gt_list, gt_doc_ids, pred_list, pred_doc_ids):
    ...
    pred_cause_map = {}  # key = (doc_id, clause_id)
    for i, (_, c, _) in enumerate(pred_list):
        pred_cause_map[(pred_doc_ids[i], str((pred_doc_ids[:i+1].count(pred_doc_ids[i]))))] = c

    gt_cause_map = {}  # key = (doc_id, clause_id)
    for j, (_, gtc, _) in enumerate(gt_list):
        gt_cause_map[(gt_doc_ids[j], str((gt_doc_ids[:j+1].count(gt_doc_ids[j]))))] = gtc
